...
def fetch_hk_stocks() -> list[str]:
    """
    港股：港交所官方「可进行卖空的指定证券名单」CSV
    URL 格式：https://www.hkex.com.hk/..../ds_list{YYYYMMDD}_c.csv
    回溯最近 10 个自然日找到最新文件
    """
...
def generate_dict(names: list[str]) -> None:
    ...
    if len(names) < existing * 0.9:
        print(f"[保护] 新数据（{len(names)}条）少于现有数据（{existing}条）的 90%，放弃写入")
        raise SystemExit(0)
